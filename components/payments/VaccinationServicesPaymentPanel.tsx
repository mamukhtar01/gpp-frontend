...
export function VaccinationServicesPaymentPanel({ additionalServices }: Props) {
  ...
  const availableServicesForClient = useMemo(() => {
    ...
    const used = new Set((addedServices[activeClientId] || []).map((s) => s.id));
  ...
  const grandTotalLocal = exchangeRate ? grandTotal * exchangeRate : null;
  const hasAnyServiceSelected = Object.values(addedServices).some((arr) => arr.length > 0);
  const bigCTAEnabled = selectedCase && hasAnyServiceSelected && grandTotal > 0 && !!exchangeRate;
  ...
  async function handleCreatePayment() {
    ...
    setLoading(true);
    try {
      const totalAmountUSD = grandTotal.toFixed(2);
      const totalAmountLocal = grandTotalLocal !== null ? grandTotalLocal.toFixed(2) : totalAmountUSD;
      ...
      const qrRes = await fetch("/api/nepalpay/generateQR", {
        ...
        body: JSON.stringify({
          ...
          transactionAmount: Number(totalAmountLocal),
      ...
      if (!qrRes.ok || !qrJson?.data?.qrString || !qrJson?.data?.validationTraceId) {
      ...
      router.push(`/payments/qrcode/${selectedCase.id}?case_type=uktb&mode=additional`);
    } catch (e: unknown) {
    ...
    <div className="relative w-full bg-white p-6 md:p-8 rounded-xl shadow-sm ring-1 ring-slate-100 max-w-6xl">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground mt-2">
          <b>Instructions:</b> Select a case, add clients then add billable additional services, then generate a QR code for payment.
        </p>
      </div>
      ...
      {selectedCase ? (
        <>
            ...
            <Table>
              <TableHeader className="bg-slate-50/60">
                <TableRow>
                  ...
                  <TableHead>Vaccination Services</TableHead>
                ...
                {ukTBCases.map((client) => {
                  ...
                  return (
                    <React.Fragment key={client.id}>
                      <TableRow>
                        <TableCell className="font-mono text-[11px]">{client.id}</TableCell>
                        <TableCell>{client.First_Name} {client.Last_Name}</TableCell>
                        <TableCell>{age}</TableCell>
                        <TableCell>
                          {services.length === 0 && (
                            <span className="text-xs italic text-slate-400">No services</span>
                          )}
                            ...
                            {services.map((svc) => (
                              ...
                              >
                                ...
                                <button
                                  ...
                                  onClick={() => removeService(client.id, svc.instanceId)}
              ...
              {ukTBCases.length > 0 && hasAnyServiceSelected && (
                <TableCaption className="px-4 pb-4 text-right">
                  <div className="inline-flex flex-col items-end">
                    <span className="text-xs tracking-wide text-slate-500">Total (USD)</span>
                    <span className="text-xl font-bold">${grandTotal.toFixed(2)}</span>
                    {exchangeRate && (
                      <>
                        <span className="text-xs tracking-wide text-green-700 mt-1">Total (NPR)</span>
                  ...
                  <SelectContent className="bg-white">
                    ...
                    {availableServicesForClient.map((svc) => (
                      <SelectItem key={svc.id} value={String(svc.id)}>
                        {svc.service_type_code.service_name} • ${parseFloat(svc.fee_amount_usd).toFixed(2)}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
          ...
          <div className="flex flex-col md:flex-row gap-4 md:gap-6 justify-between items-center bg-gradient-to-br from-slate-50 to-white rounded-xl border border-slate-200 p-5 shadow-sm">
            <div className="flex flex-wrap gap-3 text-[11px] md:text-xs">
              ...
              <span className="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-1 font-medium text-slate-600">
                Services: {Object.values(addedServices).reduce((n, arr) => n + arr.length, 0)}
              </span>
              <span className="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-1 font-medium text-slate-600">
              ...
              {exchangeRate && (
                <span className="inline-flex items-center rounded-full bg-green-100 px-2.5 py-1 font-medium text-green-700">
                  Total NPR {grandTotalLocal?.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                </span>
              )}
            </div>
        ...
        <div className="mt-4 rounded-lg border border-dashed border-slate-300 bg-slate-50/40 p-10 text-center">
          <h2 className="font-medium text-slate-700 mb-2">Start by searching a case</h2>
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            Once you select a case you will be able to choose clients and add billable additional services.
          </p>
        </div>
      )}
    </div>
