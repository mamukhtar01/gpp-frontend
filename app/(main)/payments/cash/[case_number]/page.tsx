        ...
        <CardHeader className="flex flex-col items-center gap-3 pb-0">
          ...
          <div className="bg-green-50 border border-green-100 rounded-lg px-4 py-2 mt-4">
            <span className="text-sm text-green-800">
              Transaction Date:{" "}
              <b>{new Date(payment.date_of_payment).toLocaleString()}</b>
            </span>
          </div>
        </CardHeader>
        <CardContent className="space-y-10 pt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div className="space-y-5">
              <Detail
                label="Clients"
                value={<ReadingText>{payment.payerInfo}</ReadingText>}
              />
              <Detail
                label="Case Number"
                value={<ReadingText>{payment.case_number}</ReadingText>}
              />
              <Detail
                label="Payment Type"
                value={
            ...
            <div className="space-y-5">
              <Detail
                ...
                value={<ReadingText>{payment.transaction_id}</ReadingText>}
              />
              <Detail
                label="Amount (local)"
                value={
                  <span className="font-bold text-green-700">
                    {formatCurrency(payment.paidAmount || 0, "NPR")}
                  </span>
                }
              />
              <Detail
          ...
          <div className="flex gap-3 justify-between mt-4 ">
            <Link href={`/payments/status`} passHref>
            ...
            <Link
              href={`/payments/receipt?case_number=${payment.case_number}&paymentId=${payment.id}&print=true`}
              passHref
            >
              <Button size="sm" variant={"outline"}>
                Generate Receipt
